...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len)
{
  ...
  Serial.printf("%d, moisture: %s from %s, %s, %d, %d, %d, %s\n", len, payload.moisture.c_str(), payload.name.c_str(), payload.hostAddress.c_str(), payload.task, payload.type, payload.from, payload.msg);
  ...
  else
  {
    if (payload.hostAddress == hostMac)
    {
      ...
      case UPDATE_SENSOR_PIN:
      {
        ...
        {
          Serial.println("Received invalid sensor pin number.");
        }
        break;
      }
      default:
